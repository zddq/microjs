export default {} as {
  get<O extends IMiniCookieObject, K extends keyof O>(key: K): O[K];
  set<O extends IMiniCookieObject, K extends keyof O, V extends O[K]>(key: K, val: V, opts: IMiniCookieOpts): boolean;
  has<O extends IMiniCookieObject, K extends keyof O>(key: K): boolean;
  del<O extends IMiniCookieObject, K extends keyof O>(key: K): boolean;
  parse<O extends IMiniCookieObject>(cookieStr: string): O;
 ...
 * 自定义 Cookie 对象
 */
type IMiniCookieObject<O extends Record<string, any> = {}> = Record<string, any> & O;
